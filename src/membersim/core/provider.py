...
class Provider(BaseModel):
    ...
    termination_date: Optional[date] = Field(None, description="Network participation end date")

    # Panel info (for PCPs)
    accepting_patients: bool = Field(True, description="Currently accepting new patients")
    panel_size: Optional[int] = Field(None, description="Current patient panel size")
