...
class Authorization(BaseModel):
    ...
    decision_date: Optional[date] = Field(None, description="Date decision was made")

    status: str = Field(AuthorizationStatus.PENDING, description="Current status")

    # Approval details
    approved_units: Optional[int] = Field(None, description="Units/days approved")
    effective_start: Optional[date] = Field(None, description="Auth valid from")
    effective_end: Optional[date] = Field(None, description="Auth valid through")

    # Denial details
    denial_reason: Optional[str] = Field(None, description="Reason for denial")
    denial_code: Optional[str] = Field(None, description="Denial reason code")
