...
class Attribution(BaseModel):
    ...
    provider_tin: Optional[str] = Field(None, description="Provider TIN for group attribution")
    ...
    termination_date: Optional[date] = Field(None, description="Attribution end date")

    performance_year: int = Field(..., description="Contract/measurement year")

    # Risk scoring
    risk_score: Optional[Decimal] = Field(None, description="Member's risk score (RAF)")
...
class AttributionPanel(BaseModel):
    ...
    avg_risk_score: Optional[Decimal] = Field(None, description="Average RAF score")
    total_pmpm: Optional[Decimal] = Field(None, description="Total PMPM budget")
