...
class QualityMeasure(BaseModel):
    ...
    # Age eligibility
    min_age: Optional[int] = Field(None, description="Minimum age for denominator")
    max_age: Optional[int] = Field(None, description="Maximum age for denominator")
    gender: Optional[str] = Field(None, description="Required gender (M, F, or None for both)")
...
class MemberMeasureStatus(BaseModel):
    ...
    exclusion_reason: Optional[str] = Field(None, description="Why excluded from measure")
