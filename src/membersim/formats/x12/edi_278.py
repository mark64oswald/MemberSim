...
def generate_278_request(auth: Authorization, config: Optional[X12Config] = None) -> str:
    """Generate 278 authorization request."""
    return EDI278RequestGenerator(config).generate(auth)


def generate_278_response(auth: Authorization, config: Optional[X12Config] = None) -> str:
